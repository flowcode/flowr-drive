...
namespace Flower\DriveBundle\Tests\Controller;
...
class FolderControllerTest extends WebTestCase
{
    public function testCreate()
    {
        ...
        $crawler = $client->request('GET', '/drive/folder/');
        ...
        $form = $crawler->filter('form button[type="submit"]')->form(array(
            'folder[name]' => 'Lorem ipsum dolor sit amet',
            'folder[level]' => 42,
            'folder[archived]' => true,
            'folder[created]' => new \DateTime(),
            'folder[updated]' => new \DateTime(),
                    ));
        $client->submit($form);
    ...
    {
        $client = static::createClient();
        $crawler = $client->request('GET', '/drive/folder/new');
